...
avg_service <- as.data.frame(DR_test) %>% 
...
## Loop over tasks
```{r calculate_seasonality, Warning=FALSE, message=TRUE}
...
for (scenario in scenarios){
  for(i in 1:nrow(simple_so)) {       # for-loop over rows
    ...
    message(glue::glue("Check: {scenario}_{task}_{type}"))  
    if (length(target_service) >0){
      ...
      # plot(components)
      plot(cbind(observed = components$x, 
