...
pacehrh::Trace(TRUE)
pacehrh::SetInputExcelFile("config/model_inputs.xlsx")
pacehrh::SetPerAgeStats(setting)
pacehrh::SetGlobalStartEndYears(shoulderYears = shoulderYears)
...
trials <- 50
results <- pacehrh::RunExperiments(scenarioName = "ComprehensiveModel", trials = trials, debug = FALSE)

draw_comparison <- function(taskname, trials, setting){
