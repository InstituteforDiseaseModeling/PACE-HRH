...
NextEpsilons <- function(){
  ...
  obj <- setFertilityRates(initValsEpsilons, baserates * runif(length(baserates), p[1], p[2]))
  ...
  # Mortality rate annual delta
  p = pars[pars$Value == "Annual delta mortality rates",]$p
  q = pars[pars$Value == "Annual delta mortality rates",]$q
  lims = p * q * c(-1, 1)
  baserates <- getMortalityRates(deltasBase)
  e$populationChangeParameters$changeRates <-
    setMortalityRates(
      obj,
      baserates * truncnorm::rtruncnorm(
        length(baserates),
