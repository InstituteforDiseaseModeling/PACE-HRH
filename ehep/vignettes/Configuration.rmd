...
<!--- PopType wasn't covered in the video and I'm not entirely sure what this does in the model 
    ...
    other contexts. BH: I think we should remove PopType from the file and the docs, as I don't think it is functional anymore. ---> 
...
* **adminNameAsUsed**: The human-readable name for each administrative unit. You may include 
  ...
  units within that country. Provide the values in column F, which will populate the drop-down 
...
For example, the examples in the default Ethiopian example has some tasks being shifted from certain
