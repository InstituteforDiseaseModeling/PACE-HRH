...
* **Geography**: Automatically filled based on the region specified in the **RegionSelect** sheet. 
* **StartingRateInPop**: The proportion of the relevant population that receives this care. For
  example, the value for antenatal care visits is 1 because every new birth requires that care.
...
* **MinsPerContact**: The number of minutes required by the healthcare worker to complete this task
  with an individual. 
* **HoursPerWeek**: The estimated number of hours per week a healthcare workers spends on
  non-clinical tasks that are not directly related to population rates. For example, there may be a 
  few hours of administrative tasks required regardless of clinical care provided. 
...
For example, you can see that malnutrition is highest between September and January and lower the
rest of the year.
...
we recommend verifying the sum of these values. You can add seasonality curves for additional
...
whether they are offset from (occurring before or after) the seasonality curves. Offsets are
...
in the column **sheet_Cadre**. It's very important that every task value indicator appears in
the **Cadres** sheets. 
...
changed, but the must be consistent across all sheets. The total column isn't fed into the model,
but is a visual check to be sure the staff assignments sum to 100% correctly. 
